...
- `cona grep <pattern> [-i] [--limit 50]` — substring **literal** search (NOT a regex) over code files only; each hit shows its enclosing symbol (`file:line (in Class.method): …`) so you can jump straight to `show`
