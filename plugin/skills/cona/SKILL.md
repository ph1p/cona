...
- `cona grep <pattern> [-i] [--regex] [--limit 50]` — search over code files only; each hit shows its enclosing symbol (`file:line (in Class.method): …`) so you can jump straight to `show`. **Literal by default** — `foo.bar` matches that text, not "any char"; pass `--regex` (`-e`) for a real regular expression (Rust regex syntax). An invalid regex is an error, never a silent literal search. Add `--include-deps` to also search dependency trees (`node_modules`, `vendor`, `target`, `.venv`) — they are never indexed, so this is the only way to read a library's source; hits there carry no symbol label unless the file parses
...
- `cona agents install|uninstall [names…] [--all] [--global]` — inject/remove agent configs (guide + MCP entry); no names = autodetect installed, `cona agents status` lists every supported harness
- `cona uninstall [--purge]` / `./uninstall.sh` — remove binary, hooks and every agent config cona wrote (`--purge` deletes ~/.cona)
